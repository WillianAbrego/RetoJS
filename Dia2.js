...
_ delante de la palabra, por ejemplo _playstation, que significa 
que está tachado y no se tiene que contar.

Transforma el texto a un objeto que contenga el nombre de cada regalo y las veces que aparece. 
Por ejemplo, si tenemos el texto:

const carta = 'bici coche balón _playstation bici coche
...
const listGifts = (carta) => {
  ...
  return regalos.reduce((countWords, words) => {
    countWords[words] = ++countWords[words] || 1;
    if (words.includes("_")) {
      delete countWords[words];
    }
    return countWords;
  }, {});
};
